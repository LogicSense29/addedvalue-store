...
export default function AdminStores() {
    ...
    const toggleIsActive = async (storeId, currentActive) => {
        try {
            const res = await fetch('/api/admin/stores', {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ storeId, isActive: !currentActive })
            })
            const data = await res.json()
            if (data.success) {
                setStores(prev => prev.map(s => s.id === storeId ? { ...s, isActive: !currentActive } : s))
